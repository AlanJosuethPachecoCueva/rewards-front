<template>
    ...
    <div class="containerShowRewards">
        ...
        <div class="search-container">
            <input type="text" v-model="searchQuery" placeholder="Buscar por título o descripción" class="search-bar" />
        </div>
        <div class="card-grid">
            <div v-for="reward in filteredRewards" :key="reward.url" class="cardShowRewards">
                <div class="card-image-container">
                    ...
                    <button class="redeem-button" @click="reedem(reward.metadata[0].id, reward)">Redimir</button>
                </div>
                <div class="card-body">
...
export default {
    ...
    computed: {
        filteredRewards() {
            return this.rewards.filter((reward) => {
                ...
                const description = reward.metadata[0].metadata.description.toLowerCase();
    ...
    methods: {
        reedem(rewardType, reward) {
            ...
            this.$router.push({ name: 'reedemReward', query: { reward: JSON.stringify(reward) } });
